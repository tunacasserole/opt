class Opt::Gen::AppWorker < Opt::Gen::Worker
...
PREP
  ...
  def self.process(model)
    initiate " "#{}"Creating the indexing section"
    ...
    contents = process_model(model, contents) || contents
      ...
      explorer.column_meta.each do |c|
        ...
        if attr.mapping_type == 'DIRECT'
          ...
          if v
            ...
            contents << "    #{search_type.downcase.ljust(7)}  :#{attr.attribute_name}\n"
            #puts "    #{search_type.downcase.ljust(7)}  :#{attr.attribute_name}\n"
          end
        else 
          ### MAPPED ATTRIBUTES ####
